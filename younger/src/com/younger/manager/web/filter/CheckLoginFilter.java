package com.younger.manager.web.filter;
        ...
        if (null != exceptUrls && exceptUrls.length > 0) {
            for (int i = 0; i < exceptUrls.length; i++) {
                if (uri.equals(contextPath + exceptUrls[i])) {
                    chain.doFilter(request, response);
                    return;
                } else if (exceptUrls[i].indexOf('?') > 0) {
                    String url = exceptUrls[i].substring(0, exceptUrls[i].indexOf('?'));
                    if (uri.equals(contextPath + url)) {
                        String parameters = exceptUrls[i].substring(exceptUrls[i].indexOf('?') + 1);
